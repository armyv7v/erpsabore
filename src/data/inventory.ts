...
function resolveImageUrlByCatalogOrder(productIndex: number) {
  ...
  for (const page of CATALOG_PAGES) {
    ...
    const pageEnd = cumulative + page.products;

    if (productIndex < pageEnd) {
      const indexInPage = productIndex - pageStart;
      const ratio = indexInPage / Math.max(page.products - 1, 1);
      const imageIndex = Math.min(page.images.length - 1, Math.floor(ratio * page.images.length));
      return page.images[imageIndex];
    }
