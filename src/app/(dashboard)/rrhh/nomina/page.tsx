...
import { mockPayrollSummary, mockPayrollEmployees, PayrollEmployee } from '@/data/payroll';
...
interface RichEmployee extends PayrollEmployee {
  grossAmount: number;
  afp: number;
  salud: number;
  afc: number;
...
export default function PayrollPage() {
  ...
  const initialRichEmployees: RichEmployee[] = useMemo(() => {
    const ruts = ["15.420.913-K", "18.254.098-4", "16.892.411-5"];
    return mockPayrollEmployees.map((emp, index) => {
      const gross = Math.round(emp.netAmount / 0.8);
  ...
  const currentSummary = useMemo(() => {
    const corporateBaseHaberes = 6600000;
    const corporateBaseDescuentos = 1250000;
    ...
    return {
      month: mockPayrollSummary.month,
      totalHaberes,
      growthHaberes: mockPayrollSummary.growthHaberes,
      totalDescuentos,
      sueldoLiquidoTotal,
      descuentosDetails: [
        { name: 'AFP (10% + Comisión)', amount: totalAFP, percentage: 53, colorClass: 'bg-primary' },
        { name: 'Salud (7% Fonasa/Isapre)', amount: totalSalud, percentage: 37, colorClass: 'bg-primary/70' },
        { name: 'Seguro Cesantía (AFC)', amount: totalAFC, percentage: 10, colorClass: 'bg-primary/40' }
      ]
    };
  }, [employees]);
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
          <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
            ...
            <button 
              ...
              className="text-sm font-bold text-primary hover:text-primary/90 flex items-center gap-1.5 bg-primary/5 dark:bg-primary/10 px-3 py-1.5 rounded-lg hover:bg-primary/10 transition-colors"
            >
            ...
            <ul className="space-y-6">
              <li className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Vence el 13 de Septiembre • 100% obligatorio</p>
                </div>
              </li>
              <li className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Vence el 20 de Septiembre • IVA e Imposiciones de Retención</p>
                </div>
              </li>
            </ul>
          </div>
